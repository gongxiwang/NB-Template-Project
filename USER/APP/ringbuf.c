...
//    ASSERT(pucData != NULL);
    ...
    {
        ((volatile unsigned char *)pucData)[ulTemp] = RingBufReadOne(ptRingBuf);
    }
}
...
//    ASSERT(pucBuf != NULL);
    ...
    //
    ptRingBuf->ucWritting = 0;
    ptRingBuf->ulSize = ulSize;
    ptRingBuf->pucBuf = pucBuf;
    ptRingBuf->ulWriteIndex = ptRingBuf->ulReadIndex = 0;
}

void RingBufWriteEnter(tRingBufObject *ptRingBuf)
{
    ptRingBuf->ucWritting = 1;
}

unsigned char RingBufWriteStatus(tRingBufObject *ptRingBuf)
{
    return ptRingBuf->ucWritting;
}

void RingBufWriteExit(tRingBufObject *ptRingBuf)
{
    ptRingBuf->ucWritting = 0;
}

void
RingBufClear(tRingBufObject *ptRingBuf)
{
//    ASSERT(ptRingBuf != NULL);
...
unsigned char
RingBufPeekOne(tRingBufObject *ptRingBuf, unsigned long *pulPeekReadIndex)
{
    unsigned char ucTemp;
...
//    ASSERT(ptRingBuf != NULL);
    ...
    ucTemp = ptRingBuf->pucBuf[*pulPeekReadIndex];
    UpdateIndexAtomic(pulPeekReadIndex, 1, ptRingBuf->ulSize);
    return(ucTemp);
}

void
RingBufPeek(tRingBufObject *ptRingBuf, unsigned char *pucData,
               unsigned long ulLength)
{
    unsigned long 			ulTemp;
	volatile unsigned long 	ulPeekReadIndex;
...
//    ASSERT(pucData != NULL);
	...
	ulPeekReadIndex = ptRingBuf->ulReadIndex;
    for(ulTemp = 0; ulTemp < ulLength; ulTemp++)
    {
        ((unsigned char *)pucData)[ulTemp] = RingBufPeekOne(ptRingBuf, (unsigned long *)&ulPeekReadIndex);
    }
}
